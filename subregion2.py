...
def subregion_orig(dirin=None, fittemp=None, outn=None, montage=True,correct=True,xy=True,RADEC=False,wcsra=True, GAL=False):
    """
    subregion(dirin=None, fittemp=None, outn=None)
    Crea una subregion de una imagen a partir de las coordenadas de un template
    dado, de debe ingresar un template:
    ...
    for name in listafits:
        ...
        minmax1=wcs1.getImageMinMaxWCSCoords()
        minmax2=wcs2.getImageMinMaxWCSCoords()
        centro2=wcs1.getCentreWCSCoords()
        
        if RADEC==True:
            minra=ast.astCoords.convertCoords('J2000','GALACTIC',minmax1[0],minmax1[2],2000.)
            maxra=ast.astCoords.convertCoords('J2000','GALACTIC',minmax1[1],minmax1[3],2000.)
            minmax1=[minra[0],maxra[0],minra[1],maxra[1]]
        if GAL==True:
            minra=ast.astCoords.convertCoords('GALACTIC','J2000',minmax1[0],minmax1[2],2000.)
            maxra=ast.astCoords.convertCoords('GALACTIC','J2000',minmax1[1],minmax1[3],2000.)
            minmax1=[minra[0],maxra[0],minra[1],maxra[1]]
        
        print minmax1, minmax2
        if wcs2.coordsAreInImage(minmax1[0],minmax1[2]):
                print 'true 2'
                bigmap=n.append(bigmap,name)
        if wcs2.coordsAreInImage(minmax1[0],minmax1[3]):
                bigmap=n.append(bigmap,name)
        if wcs2.coordsAreInImage(minmax1[1],minmax1[2]):
                bigmap=n.append(bigmap,name)
        if wcs2.coordsAreInImage(minmax1[1],minmax1[3]):
                bigmap=n.append(bigmap,name)
        if wcs2.coordsAreInImage(centro2[0],centro2[1]):
                bigmap=n.append(bigmap,name)
    print bigmap
    ...
    if wcsra==False:
        ...
        ra1e,dec1e=wcs1.getCentreWCSCoords()
